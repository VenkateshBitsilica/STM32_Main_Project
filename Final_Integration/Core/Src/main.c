...
char        buf[45];
const char *file_name = "GPS_data.txt";
bool flag;
...
void read_from_SD(void);
void write_to_SD(float lat, float lon);
void un_mount(void);
...
int main(void)
{
  ...
  {
     printf("TIM2 Interrupt failed...!\n");
  }

  if (HAL_UART_Receive_IT(&huart3, &rxData, 1) != HAL_OK)
  {
     printf("UART3 Interrupt failed...!\n");
  }
  ...
  while (1)
  {
	  ...
	  {
		  write_to_SD(Latitude, Longitude);
		  start_time = HAL_GetTick();
		  if(flag)
		  {
			  read_from_SD();
			  //send_sms("+919642593997", buf);
			  flag = false;
		  }
	  }
    /* USER CODE END WHILE */
...
static void MX_TIM2_Init(void)
{
  ...
  htim2.Init.Period = 59999;		//599999 for 10min delay
...
void SD_Card_Init( void )
{
    ...
    //Open the file
    fres = f_open(&fil, file_name, FA_WRITE | FA_READ | FA_OPEN_APPEND);
    if(fres != FR_OK)
    {
...
void write_to_SD(float lat, float lon)
{
	char input_buf[50];
      ...
      sprintf(input_buf,"Latitude : %f , Longitude : %f\n",lat,lon);
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART3)
    {
        if (huart->ErrorCode != HAL_UART_ERROR_NONE)
        {
            ...
            {
                __HAL_UART_CLEAR_OREFLAG(huart);  // Manually clear Overrun Error flag
            }
        }
