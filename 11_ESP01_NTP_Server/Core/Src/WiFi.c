...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    uint8_t rxBuffer[200] = {0};
	 ...
	 ntpPacket[0] = 0x1B;  // NTP request header
	...
	// Set Wi-Fi mode to Station
	  ...
	  // 3. Send CIPSEND of 48 bytes
	  sprintf(ATcmd, "AT+CIPSEND=%d\r\n", 48);
	  HAL_UART_Transmit(&huart1, (uint8_t *)ATcmd, strlen(ATcmd), 1000);
	  ESP_WaitForResponse(">", 3000);

	  // 4. Send 48-byte NTP packet
	  HAL_UART_Transmit(&huart1, ntpPacket, 48, 1000);
	  ESP_WaitForResponse("SEND OK", 3000);

	  // 5. Wait and receive 48-byte response
	  memset(rxBuf, 0, sizeof(rxBuf));
	  HAL_UART_Receive(&huart1, rxBuf, sizeof(rxBuf), 5000);
